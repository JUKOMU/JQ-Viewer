...
public class ForegroundServiceRevisionInstrumentedTest {
    ...
    public void pdfStaleStopDoesNotCloseNewerUpdate() {
        ...
        try {
            ExportForegroundService.update(context, pdfSnapshot(revision, 1));
            waitForService(context, ExportForegroundService.class, true);

            ExportForegroundService.update(context, pdfSnapshot(revision + 2, 2));
            ExportForegroundService.update(context, pdfSnapshot(revision + 1, 0));

            waitForService(context, ExportForegroundService.class, true);
        } finally {
            ExportForegroundService.update(context, pdfSnapshot(revision + 3, 0));
            waitForService(context, ExportForegroundService.class, false);
        }
    }
    ...
    private static ExportForegroundService.Snapshot pdfSnapshot(int revision, int activeCount) {
        return new ExportForegroundService.Snapshot(
            1,
            revision,
            activeCount,
