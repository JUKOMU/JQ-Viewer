...
public class ExportService {
    ...
    private ExportService(Context context) {
        this(context, createExecutor(), ExportForegroundService::update, null);
    }
    ...
    ExportService(Context context, ExecutorService executor,
        ...
        this.notif = new ExportNotificationHelper(this.context);
    ...
    public static synchronized ExportService getInstance(Context context) {
        if (instance == null) {
            instance = new ExportService(context.getApplicationContext());
        }
        return instance;
    }
     ...
     */
    public synchronized void attachEventSink(ExportEventSink sink) {
     ...
     */
    public synchronized void detachEventSink(ExportEventSink expected) {
    ...
    public void reconcileOnStartup() {
        ...
        for (int index = 0; index < before.length(); index++) {
            JSONObject task = before.optJSONObject(index);
            if (task != null && ("queued".equals(task.optString("status"))
                || "running".equals(task.optString("status"))
                || "cancelling".equals(task.optString("status")))) {
                newlyInterrupted.add(task.optString("exportId"));
            }
        }
        localFileStore.markActiveTasksInterrupted();
        JSONArray tasks = localFileStore.getAllExportTasks();
        for (int index = 0; index < tasks.length(); index++) {
            ...
            if (task != null && "interrupted".equals(task.optString("status"))) {
                ...
                if (newlyInterrupted.contains(task.optString("exportId"))) {
                    notif.showError(NotificationIds.exportTask(notificationCounter.getAndIncrement()),
                        task.optString("exportId"), task.optString("chapterTitle"),
                        "interrupted", task.optString("errorMessage"));
                }
            }
    ...
    public JSONObject getManagementState() {
        return localFileStore.getManagementState();
    }
    ...
    public JSONObject getExportTasksPage(String format, String status, String cursor, int limit) {
        ...
        return localFileStore.getExportTasksPage(format, status, cursor, limit);
    }

    public JSONObject getExportTask(String exportId) {
        return localFileStore.getExportTask(exportId);
    }

    public JSONObject cancelExport(String exportId) {
        return localFileStore.requestExportCancel(exportId);
    }

    public synchronized JSONObject retryExport(String exportId, boolean allowOverwrite) throws Exception {
        JSONObject task = localFileStore.getExportTask(exportId);
        if (task == null) {
            throw LocalFileOperationException.notFound("导出任务不存在");
        }
        JSONArray persistedChapters = localFileStore.getExportChapters(exportId);
        JSONArray persistedVolumes = localFileStore.getExportVolumes(exportId);
        ...
        if (!acquireJobLocks(job)) {
            throw LocalFileOperationException.conflict("相同章节已有任务正在运行");
        }
        if (!localFileStore.prepareExportRetry(exportId, allowOverwrite)) {
    ...
    public synchronized boolean deleteExportTask(String exportId) throws IOException {
        ...
        cleanupStagingDirectoryQuietly(task);
        if (!localFileStore.deleteExportTask(exportId)) {
            throw new IllegalStateException("活动导出任务不能删除");
        }
        return true;
    }
    ...
    private void reserveExport(ExportJob job, ExportPreflight preflight, String batchId,
        ...
        localFileStore.reserveExport(task, chapters, volumes);
    ...
    private void cleanupKnownArtifacts(String exportId, boolean strict) throws IOException {
        JSONArray volumes = localFileStore.getExportVolumes(exportId);
        IOException failure = null;
        for (int index = 0; index < volumes.length(); index++) {
            try {
                ExportArtifactCleaner.cleanupKnownVolume(volumes.getJSONObject(index));
            } catch (Exception error) {
    ...
    private static boolean usesSafTargetFolder(String targetFolderRef) {
        try {
            LocalFileRef.Parsed parsed = LocalFileRef.parse(targetFolderRef);
            return parsed.kind == LocalFileRef.Kind.FOLDER
                && parsed.provider == LocalFileRef.Provider.SAF;
    ...
    private JSONObject updateProgress(String exportId, String status, String phase,
        ...
        JSONObject snapshot = localFileStore.updateExportProgress(exportId, status, phase,
    ...
    private void updateTerminalProgress(ExportJob job, ExportPreflight preflight, String status,
        ...
        JSONObject current = localFileStore.getExportTask(job.exportId);
        ...
        try {
            for (ExportJob job : jobs) {
                ...
                try {
                    ...
                    JSONObject task = localFileStore.getExportTask(job.exportId);
                    ...
                    notif.showError(NotificationIds.exportTask(notificationCounter.getAndIncrement()),
                        job.exportId, job.chapterTitle, "failed", failure.userMessage);
                    cleanupStagingDirectoryQuietly(job);
                    releaseJobLocksAndUpdate(job);
                    JSONObject task = localFileStore.getExportTask(job.exportId);
    ...
    private void executeBatch(int batchId, List<QueuedExportJob> queuedJobs) {
        ...
        try {
            wakeLock = acquireExportWakeLock(batchId, queuedJobs.size());
            for (QueuedExportJob queuedJob : queuedJobs) {
                ...
                try {
                    publishExportForeground(sessionId, job, "准备导出", 0, 0, 0, 0);
                    ...
                    if ("partial".equals(status)) {
                        notif.showError(notificationId, job.exportId, job.chapterTitle,
                            status, "导出已取消，部分分卷已完成");
                    }
                } catch (Exception e) {
                    fail++;
                    ...
                    notif.showError(notificationId, job.exportId, job.chapterTitle, "failed",
                        "内部错误: " + t.getClass().getSimpleName());
                } finally {
                    cleanupStagingDirectoryQuietly(job);
                    releaseJobLocksAndUpdate(job);
    ...
    private void exportJob(ExportJob job, ExportPreflight preflight, int baseNotificationId,
        ...
        for (int volumeIndex = 0; volumeIndex < volumes.size(); volumeIndex++) {
            if (localFileStore.isCancelRequested(job.exportId)) throw new ExportCancelledException();
            ...
            localFileStore.markVolumeWriting(job.exportId, volumeNumber);
            ...
            PdfBoxExportWriter.ProgressListener progress = new PdfBoxExportWriter.ProgressListener() {
                ...
                public void onPageWritten(int currentPage) {
                    ...
                    if (localFileStore.isCancelRequested(job.exportId)) {
                ...
                public void onFinalizing() {
                    publishExportForeground(sessionId, job, "写入文件", volume.end, total,
                        volumeNumber, volumeCount);
                }
            };
            OutputReport report;
            ...
            if (firstOutputFileRef == null) {
                ...
                firstOutputFileName = ExportTargetPath.basename(volume.targetName);
            }
            localFileStore.completeVolumeAndRegisterFile(job.exportId, volumeNumber,
                outputFileRef, volume.displayPath, ExportTargetPath.basename(volume.targetName),
        ...
        notif.showComplete(
            ...
            firstOutputFileName == null
                ? ExportTargetPath.basename(firstVolume.targetName) : firstOutputFileName,
            firstOutputFileRef,
            detail
        );
    ...
    private String publishVolume(ExportJob job, ExportVolume volume) throws IOException {
        ...
        if (target.provider == LocalFileRef.Provider.PATH) {
            return LocalFileRef.createPathFileRef(volume.file.getCanonicalPath());
        }

        DocumentFile folder = LocalFileRefResolver.documentFile(context, job.targetFolderRef);
        if (folder == null || !folder.isDirectory() || !folder.canWrite()) {
            throw new IOException("目标目录不可写，请重新授权");
        }
        List<String> segments = ExportTargetPath.segments(volume.targetName);
        ...
        return LocalFileRef.createSafFileRef(destination.getUri().toString());
    }

    // ---- 任务锁与预检 ----

    private boolean acquireJobLocks(ExportJob job) {
        String taskKey = ExportJobValidator.taskKey(job);
        List<String> chapterKeys = ExportJobValidator.chapterResourceKeys(job);
    ...
    private ExportPreflight preflight(ExportJob job) throws IOException {
        ...
        if (targetRef.provider == LocalFileRef.Provider.SAF) {
            DocumentFile folder = LocalFileRefResolver.documentFile(context, job.targetFolderRef);
    ...
    static List<ExportVolume> buildVolumes(File pdfFile, int totalPages, int splitPages,
        ...
        String normalizedTargetName = ExportTargetPath.normalize(targetName);
        ...
        for (int index = 0; index < volumeCount; index++) {
            ...
            String volumeTargetName = volumeCount > 1
                ? ExportTargetPath.withVolumeSuffix(normalizedTargetName, start + 1, end)
    ...
    static DocumentFile findSafTarget(DocumentFile folder, String targetName) throws IOException {
        ...
        List<String> segments = ExportTargetPath.segments(targetName);
        DocumentFile current = folder;
        for (int index = 0; index < segments.size() - 1; index++) {
    ...
    private void checkExportCancelled(String exportId) throws ExportCancelledException {
        if (localFileStore.isCancelRequested(exportId)) {
    ...
    private void cleanupPublishedOutput(ExportJob job, ExportVolume volume,
        ...
        try {
            LocalFileRef.Parsed parsed = LocalFileRef.parse(outputFileRef);
            if (parsed.kind != LocalFileRef.Kind.FILE) {
                return;
            }
            if (parsed.provider == LocalFileRef.Provider.SAF) {
                cleanupCreatedSafDestination(
                    LocalFileRefResolver.documentFile(context, outputFileRef),
                    original
                );
                return;
            }
     ...
     */
    static void cleanupOwnedPathOutput(LocalFileStore localFileStore, File volumeFile,
    ...
    private File resolveOutputWorkFile(ExportJob job, LocalFileRef.Parsed targetRef)
        throws IOException {
        if (targetRef.provider == LocalFileRef.Provider.PATH) {
            File folder = new File(targetRef.payload).getCanonicalFile();
            File output = new File(folder, ExportTargetPath.normalize(job.targetName)).getCanonicalFile();
        ...
        String name = ExportTargetPath.basename(job.targetName);
    ...
    WakeLockHandle acquireExportWakeLock(int batchId, int jobCount) {
        WakeLockHandle wakeLock = null;
        try {
    ...
    private void releaseJobLocksLocked(ExportJob job) {
        activeTaskKeys.remove(ExportJobValidator.taskKey(job));
        activeChapterKeys.removeAll(ExportJobValidator.chapterResourceKeys(job));
    }
    ...
    private void publishForegroundLocked(int sessionId, int activeCount, int queueRemaining,
        ...
        foregroundPublisher.publish(
            context,
            new ExportForegroundService.Snapshot(
                sessionId,
                ++foregroundRevision,
                activeCount,
                queueRemaining,
    ...
    interface ForegroundPublisher {
        void publish(Context context, ExportForegroundService.Snapshot snapshot);
    }
