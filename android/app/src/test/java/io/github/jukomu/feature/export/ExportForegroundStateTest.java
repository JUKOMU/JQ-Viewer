...
public class ExportForegroundStateTest {

    @Test
    public void foregroundServiceRejectsOnlyOlderRevisions() {
        assertTrue(ExportForegroundService.isStaleRevision(4, 5));
        assertFalse(ExportForegroundService.isStaleRevision(5, 5));
        assertFalse(ExportForegroundService.isStaleRevision(6, 5));
    }

    @Test
    public void newlyQueuedTasksDoNotReplaceRunningTaskIdentity() {
        ExportForegroundState state = new ExportForegroundState();
        state.updateRunning("running", 7, "正在导出的本子", "正在导出", 12, 80, 1, 2);

        ExportForegroundState.Snapshot snapshot = state.snapshot(21);
    ...
    public void queuedOnlySnapshotHasNoCancelTarget() {
        ...
        ExportForegroundState.Snapshot snapshot = state.snapshot(20);
