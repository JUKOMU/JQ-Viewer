...
public class ExportServiceTest {
    ...
    public void buildsSingleVolumeWhenSplitIsDisabled() throws Exception {
        ...
        List<ExportService.ExportVolume> volumes =
            ExportService.buildVolumes(output, 205, 0);
    ...
    public void buildsVolumesFromTheWholeMergedPageRange() throws Exception {
        ...
        List<ExportService.ExportVolume> volumes =
            ExportService.buildVolumes(output, 205, 100);
    ...
    public void estimatesOriginalExportSpaceFromWorstVolumeAndExistingFinalDelta()
        ...
        File output = new File(temporaryFolder.getRoot(), "merged.pdf");
        List<ExportService.ExportVolume> volumes =
            ExportService.buildVolumes(output, 4, 2);
        ...
        long requiredBytes = ExportService.estimateRequiredBytesForExport(
            volumes,
            images,
            true
        );
    ...
    public void estimatesCompressedVolumeSpaceWithLegacyInputByteBaseline() {
        ...
        long requiredBytes = ExportService.estimateRequiredBytesForVolume(images, false);
    ...
    public void rejectsExistingOutputWithoutOverwritePermission() throws Exception {
        ...
        List<ExportService.ExportVolume> volumes =
            ExportService.buildVolumes(output, 10, 0);

        IOException error = assertThrows(IOException.class,
            () -> ExportService.ensureOverwriteAllowed(volumes, false));

        assertTrue(error.getMessage().startsWith("PDF_OUTPUT_EXISTS:"));
        ExportService.ensureOverwriteAllowed(volumes, true);
    }
    ...
    public void copiesSafDataAndChecksCancellationAfterFixedByteAmount() throws Exception {
        ...
        ExportService.ExportCancelledException error = assertThrows(
            ExportService.ExportCancelledException.class,
            () -> ExportService.copySafData(
    ...
    public void preservesCopyFailureAndClosesTheStreams() {
        ...
        IOException actual = assertThrows(IOException.class,
            () -> ExportService.copySafDestination(null, input, output, () -> {
            }));
    ...
    public void retryRejectsChangedVolumePath() throws Exception {
        ...
        List<ExportService.ExportVolume> volumes =
            ExportService.buildVolumes(output, 205, 100);
        List<ExportService.ExportVolume> persistedVolumes = Arrays.asList(
            volumes.get(0),
            new ExportService.ExportVolume(
                volumes.get(1).start,
                volumes.get(1).end,
