...
/**
 * 持久化 Desktop 导出队列、章节输入、分卷结果和文件库登记。
 */
public final class ExportStore {
    ...
    public ExportStore(Database database) {
    ...
    public synchronized void reserve(ReserveTask task, List<Chapter> chapters, List<Volume> volumes) {
        transaction(connection -> {
            try (PreparedStatement statement = connection.prepareStatement(
                "INSERT INTO export_tasks(export_id,batch_id,format,mode,album_id,album_title,"
                ...
                statement.setString(index++, task.format());
            ...
            try (PreparedStatement statement = connection.prepareStatement(
                "INSERT INTO export_task_chapters(export_id,sequence,album_id,chapter_id,"
            ...
            try (PreparedStatement statement = connection.prepareStatement(
                "INSERT INTO export_task_volumes(export_id,volume_index,start_page,end_page,"
    ...
    public synchronized Page list(
        ...
        List<ExportTaskResponse> tasks = new ArrayList<>();
        ...
        if (tasks.size() > limit) {
            ...
            ExportTaskResponse last = tasks.get(tasks.size() - 1);
    ...
    public synchronized List<Chapter> chapters(String exportId) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(
            "SELECT * FROM export_task_chapters WHERE export_id=? ORDER BY sequence")) {
            statement.setString(1, exportId);
            try (ResultSet rows = statement.executeQuery()) {
    ...
    public synchronized List<Volume> volumes(String exportId) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(
            "SELECT * FROM export_task_volumes WHERE export_id=? ORDER BY volume_index")) {
            statement.setString(1, exportId);
            try (ResultSet rows = statement.executeQuery()) {
    ...
    public synchronized boolean hasActiveChapterConflict(List<Chapter> chapters) {
        String sql = "SELECT 1 FROM export_task_chapters c JOIN export_tasks t "
    ...
    public synchronized ExportTaskResponse claim(String exportId) {
        long now = System.currentTimeMillis();
        try (PreparedStatement statement = connection.prepareStatement(
            "UPDATE export_tasks SET status='running',phase='preparing',started_at=?,"
    ...
    public synchronized ExportTaskResponse requestCancel(String exportId) {
        ExportTaskResponse current = find(exportId);
        ...
        if ("queued".equals(current.status())) {
            ...
            sql = "UPDATE export_tasks SET status='cancelling',phase='cancelling',"
    ...
    public synchronized boolean isCancellationRequested(String exportId) {
        try (PreparedStatement statement = connection.prepareStatement(
            "SELECT cancel_requested,status FROM export_tasks WHERE export_id=?")) {
            statement.setString(1, exportId);
            try (ResultSet rows = statement.executeQuery()) {
                if (!rows.next()) return true;
    ...
    public synchronized ExportTaskResponse updateProgress(
        String exportId,
        String status,
        String phase,
        int currentPage,
        ...
        long now = System.currentTimeMillis();
        String sql = "UPDATE export_tasks SET status=?,phase=?,current_page=?,total_pages=?,"
    ...
    public synchronized void markVolumeStatus(String exportId, int volumeIndex, String status) {
        try (PreparedStatement statement = connection.prepareStatement(
            "UPDATE export_task_volumes SET status=?,updated_at=? "
    ...
    public synchronized void completeVolumeAndRegisterFile(
        ...
        transaction(connection -> {
            ...
            long now = System.currentTimeMillis();
            try (PreparedStatement statement = connection.prepareStatement(
                "UPDATE export_task_volumes SET status='completed',actual_page_count=?,"
            ...
            try (PreparedStatement statement = connection.prepareStatement(
                "INSERT INTO local_files(format,file_ref,display_path,file_name,source_type,ownership,"
                    + "chapter_link_status,album_id,album_title,cover_url,authors,is_single_episode,folder_id,file_size,"
                    ...
                    + "format=excluded.format,file_name=excluded.file_name,source_type='exported',ownership='app_created',"
                    ...
                    + "is_single_episode=excluded.is_single_episode,"
            ...
            long fileId;
            try (PreparedStatement statement = connection.prepareStatement(
                "SELECT id FROM local_files WHERE file_ref=?")) {
                statement.setString(1, outputFileRef);
                try (ResultSet rows = statement.executeQuery()) {
                    if (!rows.next()) throw new IllegalStateException("导出文件登记失败");
                    fileId = rows.getLong(1);
                }
            }
            try (PreparedStatement statement = connection.prepareStatement(
            ...
            Volume volume = volumes(exportId).stream()
                .filter(item -> item.volumeIndex() == volumeIndex)
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("导出分卷不存在"));
            int chapterStart = 0;
            int sequence = 0;
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                for (Chapter chapter : chapters) {
                    int chapterEnd = chapterStart + chapter.expectedPageCount();
                    int overlapStart = Math.max(volume.startPage(), chapterStart);
                    int overlapEnd = Math.min(volume.endPage(), chapterEnd);
                    if (overlapStart < overlapEnd) {
                        ...
                        statement.setString(5, value(chapter.chapterTitle()));
                        statement.setInt(6, chapter.sortOrder());
                        statement.setInt(7, overlapStart - volume.startPage() + 1);
                        statement.setInt(8, overlapEnd - volume.startPage());
                        statement.setInt(9, overlapEnd - overlapStart);
                        statement.addBatch();
                    }
    ...
    public synchronized int completedVolumeCount(String exportId) {
        try (PreparedStatement statement = connection.prepareStatement(
            "SELECT COUNT(*) FROM export_task_volumes WHERE export_id=? AND status='completed'")) {
            statement.setString(1, exportId);
            try (ResultSet rows = statement.executeQuery()) {
                return rows.next() ? rows.getInt(1) : 0;
            }
        } catch (SQLException exception) {
            throw failure("统计已完成导出分卷失败", exception);
        }
    }
    ...
    public synchronized boolean prepareRetry(String exportId, boolean allowOverwrite) {
        return transaction(connection -> {
            long now = System.currentTimeMillis();
            try (PreparedStatement statement = connection.prepareStatement(
                "UPDATE export_tasks SET status='queued',phase='queued',allow_overwrite=?,"
            ...
            try (PreparedStatement statement = connection.prepareStatement(
                "UPDATE export_task_volumes SET status='pending',actual_page_count=0,file_size=0,"
    ...
    public synchronized List<String> markActiveInterrupted() {
        ...
        try (PreparedStatement statement = connection.prepareStatement(
            "SELECT v.temp_path FROM export_task_volumes v JOIN export_tasks t "
        ...
        transaction(connection -> {
            try (PreparedStatement statement = connection.prepareStatement(
                "UPDATE export_task_volumes SET status='interrupted',updated_at=? "
                    + "WHERE status<>'completed' AND export_id IN (SELECT export_id "
                    + "FROM export_tasks WHERE status IN (" + ACTIVE_STATUSES + "))")) {
                statement.setLong(1, now);
                statement.executeUpdate();
            }
            try (PreparedStatement statement = connection.prepareStatement(
                "UPDATE export_tasks SET status='interrupted',phase='interrupted',"
    ...
    public synchronized List<String> activeExportIds() {
        ...
        try (PreparedStatement statement = connection.prepareStatement(
            "SELECT export_id FROM export_tasks WHERE status IN (" + ACTIVE_STATUSES + ")");
             ResultSet rows = statement.executeQuery()) {
    ...
    public synchronized boolean delete(String exportId) {
        try (PreparedStatement statement = connection.prepareStatement(
            "DELETE FROM export_tasks WHERE export_id=? AND status IN ("
                + TERMINAL_STATUSES + ")")) {
            statement.setString(1, exportId);
    ...
    private static String taskSelect() {
        return "SELECT t.*,(SELECT v.output_file_ref FROM export_task_volumes v "
            + "WHERE v.export_id=t.export_id AND v.status='completed' "
            + "ORDER BY v.volume_index LIMIT 1) AS output_file_ref,"
            + "COALESCE((SELECT v.display_path FROM export_task_volumes v "
            + "WHERE v.export_id=t.export_id AND v.status='completed' "
            + "ORDER BY v.volume_index LIMIT 1),t.display_path) AS response_display_path "
            + "FROM export_tasks t";
    }

    private static ExportTaskResponse task(ResultSet rows) throws SQLException {
        ...
        return new ExportTaskResponse(
            ...
            rows.getString("format"), rows.getString("mode"), rows.getString("album_id"),
    ...
    public record Page(List<ExportTaskResponse> tasks, String nextCursor) {
    }
