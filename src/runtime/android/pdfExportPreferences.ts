...
function readExportFolder(): PdfExportFolderSelection | null {
  try {
...
export function createAndroidPdfExportPreferencesStore(): PdfExportPreferencesStore {
  return {
    get: async () => ({
      ...defaultPdfExportPreferences(),
      exportFolder: readExportFolder(),
      directoryTemplate: readText(KEY_DIR_TEMPLATE, DEFAULT_PDF_DIRECTORY_TEMPLATE),
      fileNameTemplate: readText(KEY_NAME_TEMPLATE, DEFAULT_PDF_FILE_NAME_TEMPLATE),
    }),
