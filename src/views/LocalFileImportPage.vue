...
defineOptions({ name: 'LocalFileImportPage' })
...
// ---- 状态 ----
const files = ref<LocalFileParseItem[]>([])
...
const pendingResolvedFiles = ref<LocalFileParseItem[]>([])
...
const candidateIds = (file: LocalFileParseItem): string[] =>
  Array.from(new Set(file.editedIds?.length ? file.editedIds : file.extractedIds))

const resolvedId = (file: LocalFileParseItem): string | null => {
...
const cbzCoverUrl = (file: LocalFileParseItem) =>
...
async function applyEdit(idx: number) {
  const { parseEditedLine } = await import('@/utils/importLocalFileParse')
  const file = files.value[idx]
  const { ids, chapterSortOrderHint } = parseEditedLine(editText.value.trim())

  const updated: LocalFileParseItem = {
    ...file,
...
// ---- 卡片样式 ----
function cardClass(file: LocalFileParseItem) {
...
function statusTagText(file: LocalFileParseItem): string {
...
function statusTagClass(file: LocalFileParseItem): string {
...
async function proceedToImport(resolvedFiles: LocalFileParseItem[]) {
...
async function doImport(resolvedFiles: LocalFileParseItem[], folderId?: string) {
  loading.value = true
  try {
